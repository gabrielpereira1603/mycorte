<x-layoutClient title="Horários" :tokenCompany="$tokenCompany">
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var today = new Date();
            var URL = '{{ env('APP_URL') }}';
            var collaboratorId = parseInt('{{ $collaboratorId }}'); // Substitua pelo ID do colaborador
            var companyId = '{{ $idCompany }}'; // Substitua pelo ID da empresa
            var companyToken = '{{ $tokenCompany }}';
                    ...
                    events: function(fetchInfo, successCallback, failureCallback) {
                        ...
                        Promise.all([
                            ...
                            var agendaEvents = agendas.data.map(agenda => {
                                ...
                                if (!isScheduled) {
                                    // Não agendado e não passado
                                    eventColor = '#4CAF50'; // Cor para horários disponíveis
